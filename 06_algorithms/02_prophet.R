...
model_prophet_fit <- prophet_reg(seasonality_weekly = TRUE,
...
fit_prophet_tbl <- modeltime_table(model_prophet_fit)

calibrate_tbl <- fit_prophet_tbl %>% 
  modeltime_calibrate(testing(splits))

calibrate_tbl %>% 
  modeltime_forecast(new_data = testing(splits),#new_data = artifacts_list$data$forecast_tbl,
...
prophet_model <- model_prophet_fit$fit$models$model_1

prophet_fcst <- predict(prophet_model, newdata = training(splits) %>% 
                          rename(ds = 1, y = 2))
prophet_fcst %>% glimpse()
...
# * Model ----
model_fit_prophet_xreg <- prophet_reg(seasonality_yearly = TRUE, seasonality_weekly = TRUE) %>% 
  set_engine('prophet') %>% 
