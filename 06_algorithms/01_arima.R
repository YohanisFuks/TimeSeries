...
fit_arima_ar <- arima(x = train_tbl$optins_trans, order = c(3,0,0))
...
predict(fit_arima_ar, 3) %>% as_tibble()
 ...
 predict(fit_lm_ar, 
         newdata = tibble(
           optins_trans = c( 0.968, 1.71, 0.907 ,NA)
         ))
 
 predict(fit_lm_ar, 
         newdata = tibble(
           optins_trans = c( 1.71, 0.907, 0.7692 ,NA)
         ))
...
fitted_values <- predict(fit_lm_ar1) %>% as.numeric()
train_error_tbl <- train_tbl %>% slice(-1) %>% 
  mutate(error = optins_trans - fitted_values)
...
# * Seasonal ARIMA ----
arima(train_tbl$optins_trans, order = c(1,0,0), seasonal = list(
                                order =c(1,0,0),
                                period = 7))
...
# 2.0 ARIMA ----
model_fit_arima <- arima_reg(non_seasonal_ar = 1) %>% 
  set_engine('arima') %>% 
...
model_fit_autoarima <- arima_reg() %>% 
  ...
  fit(optins_trans ~optin_time +
        ...
        fourier_vec(optin_time, period = 365)+
        month(optin_time, label = TRUE) +
        lab_event, training(splits))



# * Calibrate ----
calibrate_tbl <- modeltime_table(
  model_fit_arima,
  model_fit_autoarima
) %>% 
  modeltime_calibrate(testing(splits)) 
...
model_fit_bestarima <- calibrate_tbl %>% 
  slice(2) %>% 
  pull(.model) %>% 
  pluck(1)

write_rds(model_fit_bestarima, '00_models/model_fit_best_arima.rds')
